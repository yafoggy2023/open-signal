...
$CATEGORIES = [
    ...
    'espionage'      => '🕵️ Шпионаж',
    'aiding_enemy'   => '⚠️ Содействие противнику',
    'secret_leak'    => '🔓 Разглашение тайны',
    'weapons_theft'  => '🔫 Хищение оружия',
    'weapons_trade'  => '💣 Незаконный оборот оружия',
    'corruption'     => '💰 Коррупция',
    'fraud'          => '🎭 Мошенничество',
    'abuse'          => '⚖️ Злоупотребление должностными полномочиями',
    'cyber'          => '💻 Преступления в сфере компьютерной информации',
    'unauth_access'  => '🔐 Несанкционированный доступ к информации',
    'extremism'      => '🚨 Экстремизм',
    'extremism_part' => '👥 Участие в экстремистской деятельности',
    'extremism_fund' => '💸 Финансирование экстремизма',
    'drugs'          => '💊 Незаконный оборот наркотических веществ',
];
...
if ($st['state'] === 'await_organ') {
    ...
    send($chatId, stepBar('await_location', $d) . "📍 <b>Место события</b>\n\nГде произошло? Укажите адрес, город или регион.\n\n"
        ...
        . "• <b>«📍 Отправить геолокацию»</b> — отправить текущее местоположение", $locKb);
    return;
}
...
if ($st['state'] === 'await_event_date') {
    ...
    // Убираем reply-клавиатуру временным сообщением, сразу удаляем его, затем шлём anon с inline-кнопками
    $tmp = send($chatId, '⌛', removeKb());
    if (!empty($tmp['result']['message_id'])) {
        tg('deleteMessage', ['chat_id' => $chatId, 'message_id' => $tmp['result']['message_id']]);
    }
    send($chatId, $anonText, $anonKb);
    return;
}
